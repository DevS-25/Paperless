...
import { examCellAPI } from '../api';
import SignatureUpload from './SignatureUpload';
import './ExamCellDashboard.css';

function ExamCellDashboard({ user, onLogout }) {
  ...
  const loadDocuments = async () => {
    try {
      setError('');
      setLoading(true);
      console.log('Loading Exam Cell documents, filter:', filter);
      const response = filter === 'pending'
        ? await examCellAPI.getPendingDocuments()
        : await examCellAPI.getAllDocuments();
      console.log('Exam Cell documents loaded:', response.data);
  ...
  const loadStats = async () => {
    try {
      const response = await examCellAPI.getAllDocuments();
      const allDocs = response.data;
      const pending = allDocs.filter(d => d.status === 'FORWARDED_TO_COE').length; // Assuming Exam Cell uses COE status or similar
  ...
  const handleApprove = async (documentId) => {
    ...
    setLoading(true);
    try {
      await examCellAPI.approveDocument(documentId);
      alert('Document approved by Exam Cell successfully!');
      loadDocuments();
      setSelectedDoc(null);
    } catch (err) {
  ...
  const handleReject = async (documentId) => {
    ...
    setLoading(true);
    try {
      await examCellAPI.rejectDocument(documentId, rejectionReason);
      alert('Document rejected by Exam Cell!');
      loadDocuments();
      setSelectedDoc(null);
      setRejectionReason('');
    } catch (err) {
  ...
  const handlePreviewDocument = async (documentId, fileName) => {
    try {
      ...
      const response = await examCellAPI.downloadDocument(documentId);
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      FORWARDED_TO_COE: { class: 'badge-pending', text: 'Pending Review' },
      APPROVED_BY_COE: { class: 'badge-approved', text: 'Approved' },
      REJECTED_BY_COE: { class: 'badge-rejected', text: 'Rejected' },
    };
      ...
      <header className="dashboard-header">
        <div className="header-content">
          <div className="header-left">
            <h1>📄 Paperless</h1>
            <span className="role-badge role-exam-cell">Exam Cell Dashboard</span>
          </div>
          <div className="header-right">
            <div className="user-info">
              ...
              <div>
                ...
                <div className="user-details">Exam Cell</div>
              </div>
            </div>
            <button
                ...
                <div key={doc.id} className="document-card card">
                  <div className="document-main">
                    ...
                    {doc.deanName && (
                      <div className="mentor-info-box">
                        <p style={{ margin: 0 }}>
                          ...
                          {doc.forwardedToCoeAt && (
                            <span className="text-light" style={{ marginLeft: '375px' }}>
                              <strong>Forwarded on:</strong> {formatDate(doc.forwardedToCoeAt)}
                            </span>
                          )}
                        </p>
                      </div>
                    ...
                    <div className="document-info">
                      {doc.forwardedToExamCellAt ? (
                        <p><strong>Requested on:</strong> {formatDate(doc.forwardedToCoeAt)}</p>
                      ) : (
