...
function RegistrarDashboard({ user, onLogout }) {
  ...
  const loadDocuments = async () => {
    try {
      setError('');
      setLoading(true);
      console.log('Loading registrar documents, filter:', filter);
      const response = filter === 'pending'
        ? await registrarAPI.getPendingDocuments()
        : await registrarAPI.getAllDocuments();
      console.log('Registrar documents loaded:', response.data);
  ...
  const loadStats = async () => {
    try {
      const response = await registrarAPI.getAllDocuments();
      const allDocs = response.data;
      const pending = allDocs.filter(d => d.status === 'FORWARDED_TO_REGISTRAR').length;
      const approved = allDocs.filter(d => d.status === 'APPROVED_BY_REGISTRAR').length;
  ...
  const handleApprove = async (documentId) => {
    ...
    setLoading(true);
    try {
      await registrarAPI.approveDocument(documentId);
      alert('Document approved by Registrar successfully!');
      loadDocuments();
      setSelectedDoc(null);
    } catch (err) {
  ...
  const handleReject = async (documentId) => {
    ...
    setLoading(true);
    try {
      await registrarAPI.rejectDocument(documentId, rejectionReason);
      alert('Document rejected by Registrar!');
      loadDocuments();
      setSelectedDoc(null);
      setRejectionReason('');
    } catch (err) {
  ...
  const handlePreviewDocument = async (documentId, fileName) => {
    try {
      ...
      const response = await registrarAPI.downloadDocument(documentId);
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      FORWARDED_TO_REGISTRAR: { class: 'badge-pending', text: 'Pending Review' },
      APPROVED_BY_REGISTRAR: { class: 'badge-approved', text: 'Final Approved' },
      REJECTED_BY_REGISTRAR: { class: 'badge-rejected', text: 'Rejected' },
    };
      ...
      <header className="dashboard-header">
        <div className="header-content">
          <div className="header-left">
            <h1>📄 Paperless</h1>
            <span className="role-badge role-registrar">Registrar Dashboard</span>
          </div>
          <div className="header-right">
            <div className="user-info">
              ...
              <div>
                ...
                <div className="user-details">Registrar</div>
              </div>
            </div>
            <button
                ...
                <div key={doc.id} className="document-card card">
                  <div className="document-main">
                    ...
                    <div className="document-info">
                      {doc.forwardedToRegistrarAt ? (
                        <p><strong>Requested on:</strong> {formatDate(doc.forwardedToRegistrarAt)}</p>
                      ) : (
