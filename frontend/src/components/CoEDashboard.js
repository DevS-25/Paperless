...
import { coeAPI } from '../api';
import SignatureUpload from './SignatureUpload';
import './CoEDashboard.css';

function CoEDashboard({ user, onLogout }) {
  ...
  const loadDocuments = async () => {
    try {
      setError('');
      setLoading(true);
      console.log('Loading CoE documents, filter:', filter);
  ...
  const handleApprove = async (documentId) => {
    ...
    setLoading(true);
    try {
      await coeAPI.approveDocument(documentId);
      alert('Document approved by CoE successfully!');
      loadDocuments();
      setSelectedDoc(null);
    } catch (err) {
  ...
  const handleReject = async (documentId) => {
    ...
    setLoading(true);
    try {
      await coeAPI.rejectDocument(documentId, rejectionReason);
      alert('Document rejected by CoE!');
      loadDocuments();
      setSelectedDoc(null);
      setRejectionReason('');
    } catch (err) {
  ...
  const handlePreviewDocument = async (documentId, fileName) => {
    try {
      ...
      const response = await coeAPI.downloadDocument(documentId);
      ...
      <header className="dashboard-header">
        <div className="header-content">
          <div className="header-left">
            <h1>📄 Paperless</h1>
            <span className="role-badge role-coe">CoE Dashboard</span>
          </div>
          <div className="header-right">
            <div className="user-info">
              ...
              <div>
                ...
                <div className="user-details">CoE</div>
              </div>
            </div>
            <button
